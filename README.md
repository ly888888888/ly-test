...
```
##2、创建各类测试用例
下面通过 API 逐一创建用例。假设接口 ID 10（标准）和 11（监控专用）。
...
###2.4 逻辑测试（检查 desktop 值）
以 funshion 品牌期望 desktop=1704 为例：
...
###2.5 监控测试（复杂 tab 检查）
以品牌 funshion 为例，使用自定义函数 check_tabs：
...
curl -X POST http://172.17.7.156:5000/api/testcases \
  ...
  -d '{
    "project": "jupiter",
    "name": "/config/homepage/v4 监控测试 - funshion tab检查",
    "api_id": 10,
    "test_type": "monitor",
    "params": {
      "isencode": {"type": "fixed", "value": 1},
